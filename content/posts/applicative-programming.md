...
operations, we can compose them idiomatically[^idiom].

[^idiom]: Idiom is a synonym for Applicative Functor
...
And this, simply, is our fully functioning blog![^blog]

[^blog]: Only if you understand someting that runs on the command line, and does not have access to the database as _fully functioning_.
...
is static analysis[^static analysis]. And this is because applicative programs, unlike monadic programs, are not dependent on
runtime values. In other words, we can know more about our program, without
evaluating it.

[^static analysis]: Static analysis, is a technique with which wecan know stuff about a program without evaluating it.
